...
### Compliance & Audit
- **SIEM audit-log streaming (Splunk HEC + Microsoft Sentinel)** — every `audit_log` row is forwarded once a minute to a configured Splunk HTTP Event Collector or Sentinel Log Analytics workspace (Azure Monitor Data Collector API, HMAC-SHA256 signed). Persistent cursor, automatic retry on transient failure, and a "Send Test Event" button verifies connectivity before enabling
